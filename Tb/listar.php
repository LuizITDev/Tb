...
if ($ARQ_TURMAS && file_exists($ARQ_TURMAS)) {
    ...
    if ($filtro !== '') {
        $turmas = array_filter($turmas, function($t) use ($needleN){
            return (strpos(mb_strtolower($t), $needleN) !== false);
        });
    }
...
<style>
  ...
  form.filtro { margin: 12px 0 20px; display: flex; flex-wrap: wrap; gap: 8px; }
...
<p class="small">
  Fonte: <code>turmas.txt</code><?= $ARQ_TURMAS && basename($ARQ_TURMAS) !== 'turmas.txt' ? ' (fallback: ' . htmlspecialchars(basename($ARQ_TURMAS), ENT_QUOTES, 'UTF-8') . ')' : '' ?>,
  uma turma por linha.
</p>
...
<table>
  ...
  <tbody>
    <?php if (empty($turmas)): ?>
      <tr><td colspan="2">Nenhuma turma encontrada.</td></tr>
    <?php else: ?>
